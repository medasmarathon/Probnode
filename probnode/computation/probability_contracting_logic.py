...
def try_contract_sum_2_nodes(
  ...
  if is_complement_pattern(node1, node2):
    return ProbabilityMeasure(node2.exp.invert())
...
def contract_product_2_nodes(
  ...
  if node1.exp is not None and node2.exp is not None:     # default is joint probability: P(A)P(B) = P(A ^ B)
    return ProbabilityMeasure(ES(node1.exp & node2.exp))
  return None
...
def is_or_probability_pattern(
  ...
  for node in [node1, node2, node3]:
    ...
    if type(additive_invert_node.exp) is AndEventSet:
    ...
    if type(additive_invert_node.exp) is AndEventSet:
      ...
      if set(other_exps) == set([additive_invert_node.exp.aux_event,
        ...
        return ProbabilityMeasure(
            additive_invert_node.exp.base_event | additive_invert_node.exp.aux_event
            )
  return None